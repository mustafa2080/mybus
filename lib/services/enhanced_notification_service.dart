...
class EnhancedNotificationService {
  ...
  Future<void> notifyBusActivation({
    ...
    try {
      ...
      await _notifyParentsOfBusActivation(
        busId: busId,
        busPlateNumber: busPlateNumber,
        adminName: adminName,
      );
  ...
  Future<void> notifyBusDeactivation({
    ...
    try {
      ...
      // إشعار المشرف المعين للحافلة
      await _notifySupervisorOfBusDeactivation(
        busId: busId,
        busPlateNumber: busPlateNumber,
        driverName: driverName,
        adminName: adminName,
      ...
      // إشعار أولياء أمور الطلاب المسكنين في الحافلة
      await _notifyParentsOfBusDeactivation(
        busId: busId,
        busPlateNumber: busPlateNumber,
        adminName: adminName,
      );
    ...
    try {
      ...
      if (supervisorId != null && supervisorId != adminId) {
        await sendNotificationToUser(
          ...
          body: 'تم تفعيل الحافلة $busPlateNumber (السائق: $driverName) من قبل $adminName\n\nيمكنك الآن استئناف العمليات والرحلات',
          type: 'bus',
          data: {
            'type': 'bus_activated',
            'busId': busId,
        ...
        debugPrint('✅ Supervisor notification sent for bus activation');
      }
    } catch (e) {
      debugPrint('❌ Error sending supervisor notification for bus activation: $e');
    }
  }

  /// إشعار المشرف بإيقاف الحافلة
  Future<void> _notifySupervisorOfBusDeactivation({
    required String busId,
    required String busPlateNumber,
    required String driverName,
    required String adminName,
    ...
    try {
      ...
      if (supervisorId != null && supervisorId != adminId) {
        ...
        debugPrint('✅ Supervisor notification sent for bus deactivation');
      }
    } catch (e) {
      debugPrint('❌ Error sending supervisor notification for bus deactivation: $e');
    }
  }

  /// إشعار أولياء الأمور بتفعيل الحافلة
  Future<void> _notifyParentsOfBusActivation({
    required String busId,
    required String busPlateNumber,
    required String adminName,
  }) async {
    try {
      // الحصول على الطلاب المسكنين في الحافلة
      ...
      for (final studentDoc in studentsSnapshot.docs) {
        ...
        if (parentId != null && parentId.isNotEmpty) {
          await sendNotificationToUser(
            userId: parentId,
            title: '✅ تم تفعيل حافلة طفلك',
            body: 'تم تفعيل الحافلة $busPlateNumber الخاصة بـ $studentName\n\nيمكن الآن استئناف الرحلات المدرسية',
            type: 'student',
            data: {
              'type': 'bus_activated',
              'busId': busId,
      ...
      debugPrint('✅ Parent notifications sent for bus activation');
    } catch (e) {
      debugPrint('❌ Error sending parent notifications for bus activation: $e');
    }
  }

  /// إشعار أولياء الأمور بإيقاف الحافلة
  Future<void> _notifyParentsOfBusDeactivation({
    required String busId,
    required String busPlateNumber,
    required String adminName,
  }) async {
    try {
      ...
      debugPrint('✅ Parent notifications sent for bus deactivation');
    } catch (e) {
      debugPrint('❌ Error sending parent notifications for bus deactivation: $e');
    }
  }
}
