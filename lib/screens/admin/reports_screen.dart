...
class _ReportsScreenState extends State<ReportsScreen> with TickerProviderStateMixin {
      ...
      children: [
        ...
        Container(
          ...
          child: TabBar(
            ...
            isScrollable: true,
            tabs: const [
              Tab(text: 'إحصائيات عامة', icon: Icon(Icons.analytics, size: 18)),
              Tab(text: 'تقرير الطلاب', icon: Icon(Icons.people, size: 18)),
              Tab(text: 'تقرير الرحلات', icon: Icon(Icons.directions_bus, size: 18)),
            ],
          ),
        ),
              ...
              children: [
                ...
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    items: ['اليوم', 'الأسبوع', 'الشهر', 'السنة'].map((period) {
              ...
              children: [
                ...
                FutureBuilder<Map<String, int>>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    return Column(
                      children: [
                        _buildStatusRow('في المنزل', data['home'] ?? 0, Colors.green),
                        _buildStatusRow('في الباص', data['onBus'] ?? 0, Colors.blue),
                        _buildStatusRow('في المدرسة', data['atSchool'] ?? 0, Colors.orange),
                        _buildStatusRow('غائب', data['absent'] ?? 0, Colors.red),
                      ],
                    );
                  },
                ),
            ...
            return Container(
              ...
              child: ListTile(
                ...
                trailing: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _getStatusColor(student.currentStatus) == Colors.green
                        ? Colors.green.shade50
                        : _getStatusColor(student.currentStatus) == Colors.orange
      ...
      child: Column(
        children: [
          Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
              color: color == Colors.blue ? Colors.blue.shade50 :
                     color == Colors.green ? Colors.green.shade50 :
                     color == Colors.orange ? Colors.orange.shade50 :
                     color == Colors.red ? Colors.red.shade50 :
                     Colors.grey.shade50,
              borderRadius: BorderRadius.circular(8),
            ),
  ...
  Future<void> _exportTripsReport() async {
    try {
      ...
      // حفظ الملف (في التطبيق الحقيقي، يمكن استخدام مكتبة مثل path_provider)
      // هنا سنعرض رسالة نجاح فقط
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('تم تصدير ${trips.length} رحلة بنجاح'),
            backgroundColor: Colors.green,
            action: SnackBarAction(
              label: 'عرض',
              onPressed: () {
                // يمكن إضافة عرض الملف هنا
